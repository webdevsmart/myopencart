...
class ModelExtensionShippingXshippingpro extends Model {
	...
	public function getQuote($address) {
		...
		foreach($cart_products as $inc=>$product) {
			...
			if ($seller_id) {
				$cart_sellers[] = $seller_id;
				$cart_products[$inc]['seller_id']=$seller_id; //store for future use
			}
			if ($product_info) {
		...
		$cart_sellers=array_unique($cart_sellers);
		$operators= array('+','-','/','*');
		...
		foreach($methods as $single_method) {
			...
			if ($xshippingpro['rate_type']=='flat') {
				...
				if ($xshippingpro['rate_type']=='no_category') {
					$target_value = count(array_intersect($applicable_category,$cart_categories));

				}
				if ($xshippingpro['rate_type']=='no_manufacturer') {
					$target_value = count(array_intersect($applicable_manufacturer,$cart_manufacturers));
				}
			 ...
			 if ($xshippingpro['equation']) {
				...
				$placholder = array('{cartTotal}','{cartQnty}','{cartWeight}', '{shipping}', '{modifier}', '{volume}', '{noOfSeller}');
				...
				$noOfSeller = count($cart_sellers);

				$replacer = array($eq_total,$eq_qnty,$eq_weight,$eq_shipping,$eq_modifier, $eq_volume, $noOfSeller);
