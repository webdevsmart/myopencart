...
class MsSellerGroup extends Model {
    ...
    public function getSellerGroupBySellerId($seller_id) {
        $sql = "SELECT 
				mss.seller_group,
				(SELECT value FROM " . DB_PREFIX . "ms_seller_group_setting WHERE seller_group_id = mss.seller_group AND name = 'slr_gr_product_number_limit') as seller_product_limit,
				(SELECT COUNT(*) FROM " . DB_PREFIX . "ms_product WHERE seller_id = " . (int)$seller_id . " AND product_status != " . (int)MsProduct::STATUS_DELETED . ") as seller_product_quantity 
		...
		foreach ($data['description'] as $language_id => $language) {
			$this->db->query("
				...
				SET `name` = '" . $this->db->escape($language['name']) . "',
					...
					`seller_group_id` = " . (int)$seller_group_id . ",
					`language_id` = " . (int)$language_id . "
			");
		}
	}
	...
	public function deleteSellerGroup($seller_group_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "ms_seller_group_setting WHERE seller_group_id = '" . (int)$seller_group_id . "'");

		// Reassign seller_group_id to default_group_id for sellers in deleted group
		$this->db->query("UPDATE `" . DB_PREFIX . "ms_seller` SET seller_group = '1' WHERE seller_group = '" . (int)$seller_group_id . "'");
	}
}
?>
