...
class ControllerMultimerchReportSalesMonth extends ControllerMultimerchReportbase {

	public function index() {
		$this->document->addScript('view/javascript/multimerch/report/sales-month.js');
		...
		$this->data['heading'] = $this->language->get('ms_menu_reports_sales_by_month');
		$this->document->setTitle($this->language->get('ms_menu_reports_sales_by_month'));
		...
		$this->data['breadcrumbs'] = $this->MsLoader->MsHelper->admSetBreadcrumbs(array(
			...
			array(
				'text' => $this->language->get('ms_menu_reports') . ' - ' . $this->language->get('ms_menu_reports_sales') . ' - ' . $this->language->get('ms_menu_reports_sales_by_month'),
				'href' => $this->url->link('multimerch/report/sales-month', '', 'SSL'),
			)
		));
		...
		$this->response->setOutput($this->load->view('multimerch/report/sales-month', $this->data));
	}

	public function getTableData() {
		$dataParams = array('period_month' => 1);
		$colMap = array(
			'date' => 'o.`date_added`'
		);
		...
		if(isset($this->request->get['date_start']) && isset($this->request->get['date_end'])) {
			$dataParams['date_start'] = strtotime($this->request->get['date_start']);
			$dataParams['date_end'] = strtotime($this->request->get['date_end']);
		}
