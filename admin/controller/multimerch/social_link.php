...
class ControllerMultimerchSocialLink extends ControllerMultimerchBase {
	...
	public function getTableData() {
		...
		foreach ($results as $result) {
			...
			$actions .= "<a class='btn btn-danger ms-delete' data-toggle='tooltip' title='".$this->language->get('button_delete')."' data-id='" . $result['channel_id'] . "' data-referrer='social_channel'><i class='fa fa-trash-o'></i></a> ";

			$columns[] = array_merge(
				$result,
				array(
					'id' => "<input type='hidden' value='{$result['channel_id']}' />" . $result['channel_id'],
					'name' => $result['name'],
	...
	public function delete() {
		...
		if(!isset($this->request->get['channel_id']) && !isset($this->request->post['selected'])) {
			$json['errors'][] = $this->language->get('ms_error_channel_deleting');
		}

		if(!isset($json['errors'])) {
			$channel_ids = isset($this->request->get['channel_id']) ?
				array($this->request->get['channel_id']) :
			...
			$this->session->data['success'] =  $this->language->get('ms_success_channel_deleting');
			$json['redirect'] = $this->url->link('multimerch/social_link', 'token=' . $this->session->data['token'], true);
		}
