$(function() {
	function initChart(chartId) {
		...
		var chartData = function() {
			...
			$.ajax({
				...
				type: 'get',
				dataType: 'json',
				url: 'index.php?route=multimerch/dashboard/getChartData&token=' + msGlobals.token + '&type=' + encodeURIComponent(type),
								...
								if (key == 'afterFit') {
									newObj['afterFit'] = function(scaleInstance) {
										...
										scaleInstance.options.ticks.callback = function(value, index, values) {
											return value.length > 18 ? value.substr(0, 15) + '...' : value;
										}
									}
								}
							});
	...
	if (document.documentElement.clientWidth > 768) {
		...
		$('#ms_top_countries_chart').attr('width', 3);
		$('#ms_top_sellers_chart').attr('width', 3);
		$('#ms_top_customers_chart').attr('width', 3);
		$('#ms_top_products_chart').attr('width', 3);
	}
	...
	if (document.documentElement.clientWidth <= 768 && document.documentElement.clientWidth > 425) {
		...
		$('#ms_top_countries_chart').attr('width', 2);
		$('#ms_top_sellers_chart').attr('width', 2);
		$('#ms_top_customers_chart').attr('width', 2);
		$('#ms_top_products_chart').attr('width', 2);
	}

	// Mobile devices
	if (document.documentElement.clientWidth <= 425) {
		$('#ms_sales_analytics_chart').attr('width', 1);
		$('#ms_top_countries_chart').attr('width', 1);
		$('#ms_top_sellers_chart').attr('width', 1);
		$('#ms_top_customers_chart').attr('width', 1);
		$('#ms_top_products_chart').attr('width', 1);
	}

	initChart('ms_sales_analytics_chart');
	initChart('ms_top_countries_chart');
	initChart('ms_top_sellers_chart');
	initChart('ms_top_customers_chart');
	initChart('ms_top_products_chart');
});